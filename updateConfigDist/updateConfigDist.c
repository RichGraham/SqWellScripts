...
int main(int argc, char *argv[])
{
  ...
  for(i=1;i<=argc-1;i++){
    ...
    while(!feof(inFilePtr)){
      fscanf(inFilePtr,"%lf %lf %lf", &x[0][j], &x[1][j], &x[2][j]);
      j++;
    }
    fclose(inFilePtr);
...
int SNorderParam(double x[][500], int N, double lambda, int *particleState, int corPoints[][500]){
  ...
  */
	...
	//____Count neighbours and make neighbour list____
	for(i=0;i<N;i++){
		particleState[i] = 2;
		k=0;
		for(j=0;j<N;j++){
	...
	//____Compute spherical harmonics____
	...
	//____Compute dot product d_ij  (to test whether particles are connected)____
	for(i=0;i<N;i++){
		for(j=0;j<Nn[i];j++){
			mqi2 = 0.0;
			mqj2 = 0.0;
	...
	//_____Classify all particles___
	for(i=0;i<N;i++){
	  for(j=0;j<Nn[i];j++){ //count connected neighbours
			if(d[i][j]-DC>0)
			  Nc[i]++;
		}
